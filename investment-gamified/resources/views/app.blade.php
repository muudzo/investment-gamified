    ...
    <script>
        ...
        async function loadUserData() {
            ...
            if (data.success) {
                ...
                document.getElementById('userBalance').textContent = Number(data.data.balance).toFixed(2);
                document.getElementById('portfolioValue').textContent = Number(data.data.total_value).toFixed(2);
        ...
        async function loadStocks() {
            ...
            stocksList.innerHTML = data.data.map(stock => `
                <div class="border rounded-xl p-4 hover:shadow-md transition">
                    ...
                    <p class="text-sm text-gray-600 mb-3">${stock.kid_friendly_description}</p>
                    <p class="text-xs text-purple-600 mb-3">💡 ${stock.fun_fact}</p>
                    ...
                    <div class="border rounded-lg p-3">
                        <div class="flex justify-between items-center">
                            ...
                            <p class="text-sm font-bold ${item.profit_loss >= 0 ? 'text-green-600' : 'text-red-600'}">
                                ${item.profit_loss >= 0 ? '+' : ''}$${Number(item.profit_loss).toFixed(2)}
                            </p>
                        </div>
                    </div>
                `).join('');
